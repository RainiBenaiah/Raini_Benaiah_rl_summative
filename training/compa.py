...
class A2CTraining:
    ...
    def __init__(self, num_hives=4, save_path="models/pg/c_model.zip"):
    ...
    def train(self):
        ...
        hyperparams = {
            ...
            "gae_lambda": 1.0,
            "vf_coef": 0.5,
            "ent_coef": 0.04,  # Increased to encourage exploration
        }
                    ...
                    try:
                        ...
                        if obs is not None:
                            ...
                            with torch.no_grad():
                                ...
                                print(f"A2C Logged entropy: {entropy:.4f}")
                    except Exception as e:
                        print(f"A2C Entropy logging error: {e}")
        ...
        if not self.policy_entropies:
            print("Warning: No A2C policy entropies logged during training")
    ...
    def record_gif(self, model, episodes=3, max_steps=1000):
        ...
        for ep in range(episodes):
            ...
            filename = f"gifs/c_episode_{ep+1}.gif"
    ...
    def plot_results(self):
        ...
        if self.episode_rewards:
            ...
            plt.legend()
            plt.savefig("plots/c_cumulative_reward.png")
            plt.close()
        else:
            print("Warning: No A2C episode rewards available for cumulative reward plot")
        ...
        if self.policy_entropies:
            ...
            plt.legend()
            plt.savefig("plots/c_training_stability.png")
            plt.close()
        else:
            print("Warning: No A2C policy entropies available for stability plot")

class PPOTraining:
    """Class to handle PPO training, evaluation, and visualization"""
    
    def __init__(self, num_hives=4, save_path="models/pg/cpo_model.zip"):
        self.save_path = save_path
        self.env = Monitor(BeehiveManagementEnv(num_hives=num_hives, render_mode="human"))
        self.eval_env = Monitor(BeehiveManagementEnv(num_hives=num_hives, render_mode="human"))
        self.renderer = BeehiveRenderer(width=1200, height=800)
    ...
    def train(self):
                    ...
                    try:
                        ...
                        if obs is not None:
                            ...
                            with torch.no_grad():
                                ...
                                entropy = dist.distribution.entropy().mean().item()
                                self.entropies.append(entropy)
                                print(f"PPO Logged entropy: {entropy:.4f}")
                    except Exception as e:
                        print(f"PPO Entropy logging error: {e}")
                return super()._on_step()
        ...
        if not self.policy_entropies:
            print("Warning: No PPO policy entropies logged during training")
    ...
    def record_gif(self, model, episodes=3, max_steps=1000):
        ...
        for ep in range(episodes):
            ...
            filename = f"gifs/cpo_episode_{ep+1}.gif"
            imageio.mimsave(filename, [frame.astype(np.uint8) for frame in frames], fps=8)
    ...
    def plot_results(self):
        ...
        if self.episode_rewards:
            ...
            plt.legend()
            plt.savefig("plots/cpo_cumulative_reward.png")
            plt.close()
        else:
            print("Warning: No PPO episode rewards available for cumulative reward plot")
        ...
        if self.policy_entropies:
            ...
            plt.legend()
            plt.savefig("plots/cpo_training_stability.png")
            plt.close()
        else:
            print("Warning: No PPO policy entropies available for stability plot")

def compare_models():
    """Train and compare A2C and PPO models, return the best one"""
    pygame.init()
    ...
    # Save best model
    best_model = a2c_model if best_model_name == "A2C" else ppo_model
    best_model_save_path = "models/pg/better_model.zip"
    best_model.save(best_model_save_path)
    print(f"Best model saved to {best_model_save_path}")

    # Plot comparison
    plt.figure(figsize=(10, 5))
    if a2c_trainer.episode_rewards:
        plt.plot(np.cumsum(a2c_trainer.episode_rewards), label="A2C Cumulative Reward")
    if ppo_trainer.episode_rewards:
        plt.plot(np.cumsum(ppo_trainer.episode_rewards), label="PPO Cumulative Reward")
    plt.xlabel("Episode")
    plt.ylabel("Cumulative Reward")
    plt.title("A2C vs PPO Cumulative Reward Comparison")
    plt.legend()
    plt.savefig("plots/pg_cumulative_reward_comparison.png")
    plt.close()
